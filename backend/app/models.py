...
class User(Base):
    ...
    role = Column(String, default="verifier") # admin, officer, verifier
...
class Document(Base):
    ...
    status = Column(String, default="Uploaded") # Uploaded, Processing, OCR Complete, Validating, Needs Verification, Verified, Failed
...
class ExtractedRecord(Base):
    ...
    # Confidence Scores as JSON string e.g. {"owner_name": "high", "survey_number": "low"}
    confidence_scores = Column(JSON, nullable=True)
    
    # Validation results
    validation_status = Column(String, default="Pending") # Passed, Failed
    validation_errors = Column(JSON, nullable=True) # Explanations of why it flagged
...
class Parcel(Base):
    ...
    area = Column(Float)
    # Storing GeoJSON polygon as JSON since we dropped PostGIS for the demo
